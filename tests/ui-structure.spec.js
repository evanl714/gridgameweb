...
test.describe('UI Structure Analysis', () => {
  ...
  test('analyze UI structure before and after new game click', async ({ page }) => {
    ...
    if (originalStatus > 0) {
      console.log('Original gameStatus text:', await page.locator('#gameStatus').textContent());
    }
    if (originalPlayer > 0) {
      console.log('Original currentPlayer text:', await page.locator('#currentPlayer').textContent());
    }
    
    // Get all elements that might contain status info
    const allStatusElements = await page.locator('[id*="status"], [class*="status"], [id*="game"], [class*="game"]').count();
    console.log('Total elements with status/game in id/class:', allStatusElements);
    ...
    if (newStatus > 0) {
      console.log('After gameStatus text:', await page.locator('#gameStatus').textContent());
    }
    if (newPlayer > 0) {
      console.log('After currentPlayer text:', await page.locator('#currentPlayer').textContent());
    }
    
    // Check for new UI elements
    const uiContainers = await page.locator('.ui-container').count();
    const gameStatusContainers = await page.locator('#gameStatusContainer').count();
    const resourceContainers = await page.locator('#resourceDisplayContainer').count();
    ...
    const statusGameElements = await page.evaluate(() => {
      const elements = document.querySelectorAll('[id*="status"], [id*="game"], [id*="Game"]');
    ...
    statusGameElements.forEach(el => {
      console.log(`- ${el.tagName}#${el.id}.${el.className}: "${el.textContent}"`);
    });
  });
});