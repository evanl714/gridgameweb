...
describe('Movement System Edge Cases', () => {
    ...
    test('should handle corner cases at grid boundaries', () => {
      ...
      corners.forEach((corner, index) => {
        ...
        const invalidMoves = [
          ...
          move.x < 0 || move.x >= 25 || move.y < 0 || move.y >= 25
        );
  ...
  describe('Collision Detection', () => {
    ...
    test('should handle multiple units clustering', () => {
      ...
      for (let x = 10; x <= 12; x++) {
        for (let y = 10; y <= 12; y++) {
          if (!(x === 11 && y === 11)) { // Skip center position
  ...
  describe('Movement Range Display', () => {
    test('should calculate valid move positions correctly', () => {
      ...
      const expectedPositions = [
        // Distance 1
        { x: 9, y: 10, cost: 1 }, { x: 11, y: 10, cost: 1 },
        { x: 10, y: 9, cost: 1 }, { x: 10, y: 11, cost: 1 },
        // Distance 2
        { x: 8, y: 10, cost: 2 }, { x: 12, y: 10, cost: 2 },
        { x: 10, y: 8, cost: 2 }, { x: 10, y: 12, cost: 2 },
        { x: 9, y: 9, cost: 2 }, { x: 9, y: 11, cost: 2 },
        { x: 11, y: 9, cost: 2 }, { x: 11, y: 11, cost: 2 }
      ];
      
      expect(validMoves.length).toBe(expectedPositions.length);
      
      expectedPositions.forEach(expected => {
        const found = validMoves.find(move => 
    ...
    test('should exclude occupied positions from valid moves', () => {
      ...
      const occupiedMove = validMoves.find(move => move.x === 11 && move.y === 10);
  ...
  describe('Performance Testing', () => {
    ...
    test('should handle large grid efficiently', () => {
      ...
      for (let i = 0; i < 100; i++) {
        gameState.getValidMovePositions(unit.id);
        gameState.calculateMovementCost(unit.id, Math.floor(Math.random() * 25), Math.floor(Math.random() * 25));
      }
  ...
  describe('Error Handling', () => {
    ...
    test('should maintain consistency after failed operations', () => {
      ...
      expect(gameState.board[originalPosition.x][originalPosition.y]).toBe(unit.id);
    });
  });
});